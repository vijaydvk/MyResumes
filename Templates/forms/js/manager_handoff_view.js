$(document).ready(function () { 
	...
	var storesDetails = [];
	$.when(getUniqueHandoffForm()).done(function(){
	...
	function getStoreForDropdown()
	{
		...
		error: function() {
			console.log("manager_handoff_view - getStoreForDropdown - Error - line 19"); 
	...
	{
		return $.ajax({
			url:'controller/index1.php?action=getUniqueHandoffForm&id='+$("#handoff_id").val(),
			type:'POST',
			success:function(data){
		...
		error: function() {
			console.log("manager_handoff_view - getUniqueHandoffForm - Error - line 34"); 
			console.log('something bad happened'); }
		}) ;		
